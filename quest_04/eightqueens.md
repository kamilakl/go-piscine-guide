...
| **Термин** | **Объяснение** |
|-------------|----------------|
| **Backtracking (бэктрекинг)** | Метод поиска, в котором мы поочерёдно пробуем варианты и “откатываемся” назад, если путь не подходит. Очень часто используется при решении рекурсивных задач, например восьми ферзей. |
| **Рекурсия (recursion)** | Функция вызывает саму себя, но с другими параметрами. Здесь — чтобы перейти к следующему столбцу. |
| **Базовый случай** | Условие, при котором рекурсия останавливается (`col == 8`). В этот момент найдено полное решение. |
| **Диагонали** | Две клетки находятся на одной диагонали, если разница по строкам равна разнице по столбцам: `|row1 - row2| == |col1 - col2|`. |
| **Функция `z01.PrintRune`** | Используется для вывода символов. Чтобы напечатать цифру из числа `n`, нужно вывести `'0' + n`. |
...
Это классический пример **рекурсивного бэктрекинга** — пошагового перебора с откатом.  
...
Такой способ разделения делает код **структурным и читаемым**.  
К подобной практике стоит **приучаться**, чтобы большие задачи можно было разбивать на небольшие, независимые части —  
и легко комбинировать их между собой.

# Quest 4.9 Eight Queens — Пошаговый разбор кода
...
Каждый вызов `placeQueen` — это **новый уровень рекурсии**,  
а после возврата функция “откатывает” позицию и пробует следующую строку.

---

## Заключение

Код реализует **чистый рекурсивный перебор** с проверкой безопасности и печатью всех возможных расстановок.  
- Рекурсия делает решение коротким и читаемым,  
- отдельные функции (`isSafe`, `printSolution`, `abs`) упрощают понимание,  
- порядок перебора обеспечивает **лексикографический вывод** (по возрастанию).  

К такой практике стоит **приучаться** —  
разделять задачу на логические блоки, давать каждой функции одну ответственность  
и писать код, который легко читать и сопровождать.
