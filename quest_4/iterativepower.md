...
|--------|------------|
| Цикл `for` | Конструкция, которая позволяет повторять действия несколько раз, пока выполняется определенное условие. Цикл `for` в Go может принимать разные формы. Подробнее: https://metanit.com/go/tutorial/2.10.php |
...
| Возврат значения (`return`) | Команда которая завершает работу функции и отдаёт результат, который она вычислила. **Примечание:** Если написать `return` внутри цикла `for`, то функция немедленно завершится, и цикл прекратит своё выполнение. `return` подразумевает полное завершение работы для функции. |
| Buffer Overflow (переполнение буфера) | Это случается когда результат вычислений выходит за пределы диапазона переменной. **Пример:** *uint8 может хранить числа 0–255. Если сложить 250 + 10, получится 260 → компьютер берет остаток от деления на 256 → 260 % 256 = 4; число “заворачивается” и становится 4.* см. https://ru.wikipedia.org/wiki/%D0%9F%D0%B5%D1%80%D0%B5%D0%BF%D0%BE%D0%BB%D0%BD%D0%B5%D0%BD%D0%B8%D0%B5_%D0%B1%D1%83%D1%84%D0%B5%D1%80%D0%B0|
